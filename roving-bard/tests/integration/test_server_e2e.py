...
def test_vlm_endpoints(server_fixture: subprocess.Popen[str]) -> None:
    ...
    # 4. Trigger download/pull for a local VLM (mock-vlm)
    pull_url = BASE_URL + "/api/ocr/vlm_pull"
    response = requests.post(pull_url, json={"model": "mock-vlm"}, headers=HEADERS, timeout=10)
    assert response.status_code == 200
    assert response.json()["status"] == "success"
    
    response = requests.get(status_url, headers=HEADERS, timeout=10)
    assert response.status_code == 200
    status = response.json()["states"]["mock-vlm"]["status"]
    assert status.startswith("downloading") or "pulling" in status
    
    # 5. Pause the download
    pause_url = BASE_URL + "/api/ocr/vlm_pause"
    response = requests.post(pause_url, json={"model": "mock-vlm"}, headers=HEADERS, timeout=10)
    assert response.status_code == 200
    assert response.json()["status"] == "success"
    ...
    response = requests.get(status_url, headers=HEADERS, timeout=10)
    assert response.status_code == 200
    assert response.json()["states"]["mock-vlm"]["status"] == "paused"
    ...
    # 7. Trigger pull for gemma-3 (skip pulling if already ready to reuse cached model)
    response = requests.get(status_url, headers=HEADERS, timeout=10)
    assert response.status_code == 200
    gemma_ready = response.json()["states"]["gemma-3"]["ready"]
